...
template<typename T> vec2<T> operator/(const vec2<T>& a,const T& b) { return vec2<T>(a.x / b,a.y / b); }
...
struct mat22 {
    ...
    explicit mat22(const vec2<T>& pr0,const vec2<T>& pr1) 
        : r0(pr0),r1(pr1) {}
...
template<int N,typename T> vec2<T> row(const mat22<T>& a) { return get<N>(a); };
template<int N,typename T> vec2<T> col(const mat22<T>& a) { return vec2<T>(get<N>(a.r0),get<N>(a.r1)); };

template<typename T> mat22<T> operator+(const mat22<T>& a,const mat22<T>& b) { return mat22<T>(a.r0+b.r0, a.r1+b.r1); }
template<typename T> mat22<T> operator/(const mat22<T>& a,const T& b) { return mat22<T>(a.r0/b, a.r1/b); }

template<typename T> vec2<T> operator*(const mat22<T>& a,const vec2<T>& b) {
    return vec2<T>(dot(row<0>(a),b),dot(row<1>(a),b));
}
...
using fmat22 = mat22<float>;

using dvec2 = vec2<double>;
using dmat22 = mat22<double>;
...
template<typename T> mat22<T> inverse(const mat22<T>& a) { return adjugate(a) / determinant(a); }
